...
$sql = 'SELECT s.question_id, s.answer_id, a.answer_id, a.hse_answer_code FROM `hse_survey_question_answer` s LEFT JOIN answers a ON s.answer_id = a.answer_id ORDER BY `id` ASC';
...
foreach ($result as $key => $value) {
    ...
    switch($value['question_id']){
        case (int) 1:
            $sex[] = $value['hse_answer_code'];
            break;
        case (int) 2:
            $age[] = $value['hse_answer_code'];
            break;
        case (int) 4:
